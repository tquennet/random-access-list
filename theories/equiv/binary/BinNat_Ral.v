...
Theorem RAL_open_sub_O : forall (l : @RAL A) (n : BN)  dl tl,
		RAL_open l n [] [] = (dl, None, tl) -> dl = rev l /\ tl = [].
Proof.
Admitted.

Lemma RAL_open_sub_aux : forall (el l : @RAL A) (en n : BN) dl t dt tl,
		RAL_open l n (rev (size el - en)) (rev el) = (dl, Some(t, dt), tl) ->
		(size (el ++ l)) - (en ++ n) = BN_inc (DCLBT_trace dt ++ 0 :: size tl).
Proof.
Admitted.

Theorem RAL_open_sub : forall (l : @RAL A) (n : BN) dl t dt tl,
